...
from constants import Errno, PAGE_SIZE, Sysno
...
def _install_base_bins(fs: BetterFS) -> None:
    ...
    sh.align_data(8)
    pathbuf = sh.db(b"\x00" * 128)
    sh.align_data(8)
    pathbuf2 = sh.db(b"\x00" * 128)
    sh.align_data(8)
    ...
    # Pipeline support: detect a single '|' token (must be space-separated).
    sh.li(T0, 0)
    sh.li(T2, -1)
    sh.label("scan_pipe")
    sh.beq(T0, S4, "pipe_scan_done")
    sh.emit(rvasm.slli(T1, T0, 3))
    sh.emit(rvasm.add(T1, S1, T1))
    sh.emit(rvasm.ld(A0, T1, 0))
    sh.emit(rvasm.lbu(A1, A0, 0))
    sh.li(A2, 124)
    sh.bne(A1, A2, "pipe_next")
    sh.emit(rvasm.lbu(A1, A0, 1))
    sh.bne(A1, 0, "pipe_next")
    sh.emit(rvasm.addi(T2, T0, 0))
    sh.jal(0, "pipe_scan_done")
    sh.label("pipe_next")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.jal(0, "scan_pipe")
    sh.label("pipe_scan_done")
    sh.blt(T2, 0, "no_pipe")

    # Split argv in-place: argv[pipe_idx] = 0; right_argv = &argv[pipe_idx+1]
    sh.beq(T2, 0, "pipe_syntax")
    sh.emit(rvasm.addi(T0, S4, -1))
    sh.beq(T2, T0, "pipe_syntax")
    sh.emit(rvasm.slli(T1, T2, 3))
    sh.emit(rvasm.add(T1, S1, T1))
    sh.emit(rvasm.sd(0, T1, 0))
    sh.emit(rvasm.addi(T2, T2, 1))
    sh.emit(rvasm.slli(T1, T2, 3))
    sh.emit(rvasm.add(30, S1, T1))

    # Build left exec path into pathbuf (S2) -> S6 (x22).
    sh.li(T0, 0)
    sh.label("pl_scan_slash")
    sh.emit(rvasm.add(A0, S5, T0))
    sh.emit(rvasm.lbu(T1, A0, 0))
    sh.beq(T1, 0, "pl_no_slash")
    sh.li(T2, 47)
    sh.beq(T1, T2, "pl_has_slash")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.jal(0, "pl_scan_slash")
    sh.label("pl_has_slash")
    sh.emit(rvasm.addi(22, S5, 0))
    sh.jal(0, "pl_path_done")
    sh.label("pl_no_slash")
    sh.li(T0, 0)
    sh.label("pl_cpy_pre")
    ...
    sh.beq(T2, 0, "pl_cpy_pre_done")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.jal(0, "pl_cpy_pre")
    sh.label("pl_cpy_pre_done")
    sh.emit(rvasm.addi(T0, T0, -1))
    sh.li(T1, 0)
    sh.label("pl_cpy_cmd")
    ...
    sh.beq(T2, 0, "pl_path_ready")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.emit(rvasm.addi(T1, T1, 1))
    sh.jal(0, "pl_cpy_cmd")
    sh.label("pl_path_ready")
    sh.emit(rvasm.addi(22, S2, 0))
    sh.label("pl_path_done")

    # Build right exec path into pathbuf2 (x28) -> x31; right argv base in x30.
    sh.emit(rvasm.ld(A0, 30, 0))
    sh.emit(rvasm.addi(A3, A0, 0))
    sh.li(T0, 0)
    sh.label("pr_scan_slash")
    sh.emit(rvasm.add(A0, A3, T0))
    sh.emit(rvasm.lbu(T1, A0, 0))
    sh.beq(T1, 0, "pr_no_slash")
    sh.li(T2, 47)
    sh.beq(T1, T2, "pr_has_slash")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.jal(0, "pr_scan_slash")
    sh.label("pr_has_slash")
    sh.emit(rvasm.addi(31, A3, 0))
    sh.jal(0, "pr_path_done")
    sh.label("pr_no_slash")
    sh.li(T0, 0)
    sh.label("pr_cpy_pre")
    sh.emit(rvasm.add(T1, 27, T0))
    sh.emit(rvasm.lbu(T2, T1, 0))
    sh.emit(rvasm.add(T1, 28, T0))
    sh.emit(rvasm.sb(T2, T1, 0))
    sh.beq(T2, 0, "pr_cpy_pre_done")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.jal(0, "pr_cpy_pre")
    sh.label("pr_cpy_pre_done")
    sh.emit(rvasm.addi(T0, T0, -1))
    sh.li(T1, 0)
    sh.label("pr_cpy_cmd")
    sh.emit(rvasm.add(A0, A3, T1))
    sh.emit(rvasm.lbu(T2, A0, 0))
    sh.emit(rvasm.add(A0, 28, T0))
    sh.emit(rvasm.sb(T2, A0, 0))
    sh.beq(T2, 0, "pr_path_ready")
    sh.emit(rvasm.addi(T0, T0, 1))
    sh.emit(rvasm.addi(T1, T1, 1))
    sh.jal(0, "pr_cpy_cmd")
    sh.label("pr_path_ready")
    sh.emit(rvasm.addi(31, 28, 0))
    sh.label("pr_path_done")
    ...
    # fork writer (left): stdout -> pipe write end
    rvlib.sys_fork(sh)
    sh.beq(A0, 0, "pipe_child1")
    rvlib.sys_close(sh, fd_reg=T1)

    # fork reader (right): stdin -> pipe read end
    rvlib.sys_fork(sh)
    sh.beq(A0, 0, "pipe_child2")
    rvlib.sys_close(sh, fd_reg=T0)

    # wait for both children (any order). laststatus becomes last reaped.
    sh.li(T2, -1)
    sh.label("pipe_wait1")
    rvlib.sys_waitpid(sh, child_pid_reg=T2, status_addr=statusbuf)
    sh.li(T1, -11)
    sh.beq(A0, T1, "pipe_wait1")
    sh.emit(rvasm.ld(26, S3, 0))
    sh.label("pipe_wait2")
    rvlib.sys_waitpid(sh, child_pid_reg=T2, status_addr=statusbuf)
    sh.li(T1, -11)
    sh.beq(A0, T1, "pipe_wait2")
    sh.emit(rvasm.ld(26, S3, 0))
    sh.jal(0, "loop")

    sh.label("pipe_child1")
    rvlib.sys_dup2(sh, oldfd_reg=T1, newfd=1)
    rvlib.sys_close(sh, fd_reg=T0)
    rvlib.sys_close(sh, fd_reg=T1)
    sh.emit(rvasm.addi(A0, 22, 0))
    ...
    sh.label("pipe_child2")
    rvlib.sys_dup2(sh, oldfd_reg=T0, newfd=0)
    rvlib.sys_close(sh, fd_reg=T0)
    sh.emit(rvasm.addi(A0, 31, 0))
    sh.emit(rvasm.addi(A1, 30, 0))
    sh.li(A2, 0)
    sh.li(A7, int(Sysno.EXECVE))
    rvlib.ecall(sh)
    rvlib.sys_write(sh, fd=1, buf=execfail, count=len(b"execve: failed\n"))
