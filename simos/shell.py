...
def _install_base_bins(fs: BetterFS) -> None:
    ...
    p_touch.li(rvlib.A1, int(O_CREAT))
    p_touch.li(rvlib.A7, int(Sysno.OPEN))
    rvlib.ecall(p_touch)
    p_touch.blt(rvlib.A0, 0, "touch_openfail")
    p_touch.emit(rvasm.addi(rvlib.S0, rvlib.A0, 0))
    p_touch.emit(rvasm.addi(rvlib.A0, rvlib.S0, 0))
    p_touch.li(rvlib.A7, int(Sysno.CLOSE))
    rvlib.ecall(p_touch)
    ...
    p_cp.label("cp_loop")
    p_cp.emit(rvasm.addi(rvlib.A0, rvlib.S0, 0))
    p_cp.li(rvlib.A1, cp_buf)
    p_cp.li(rvlib.A2, 256)
    p_cp.li(rvlib.A7, int(Sysno.READ))
    rvlib.ecall(p_cp)
    p_cp.beq(rvlib.A0, 0, "cp_done")
    p_cp.blt(rvlib.A0, 0, "cp_readfail")
    p_cp.emit(rvasm.addi(rvlib.T0, rvlib.A0, 0))

    p_cp.emit(rvasm.addi(rvlib.A0, rvlib.S1, 0))
    p_cp.li(rvlib.A1, cp_buf)
    p_cp.emit(rvasm.addi(rvlib.A2, rvlib.T0, 0))
    p_cp.li(rvlib.A7, int(Sysno.WRITE))
    rvlib.ecall(p_cp)
    ...
    p_cp.label("cp_done")
    p_cp.emit(rvasm.addi(rvlib.A0, rvlib.S0, 0))
    p_cp.li(rvlib.A7, int(Sysno.CLOSE))
    rvlib.ecall(p_cp)
    p_cp.emit(rvasm.addi(rvlib.A0, rvlib.S1, 0))
    p_cp.li(rvlib.A7, int(Sysno.CLOSE))
    rvlib.ecall(p_cp)
    ...
    p_head.li(rvlib.A1, 0)
    p_head.li(rvlib.A7, int(Sysno.OPEN))
    rvlib.ecall(p_head)
    p_head.blt(rvlib.A0, 0, "head_usage")
    p_head.emit(rvasm.addi(rvlib.S0, rvlib.A0, 0))
    p_head.li(rvlib.T2, 0)
    p_head.label("head_read")
    p_head.emit(rvasm.addi(rvlib.A0, rvlib.S0, 0))
    p_head.li(rvlib.A1, head_buf)
    p_head.li(rvlib.A2, 256)
    p_head.li(rvlib.A7, int(Sysno.READ))
    rvlib.ecall(p_head)
    ...
    p_head.emit(rvasm.add(19, rvlib.A1, rvlib.T1))
    ...
    p_head.li(rvlib.A0, 1)
    p_head.li(rvlib.A1, head_buf)
    p_head.emit(rvasm.addi(rvlib.A2, 18, 0))
    p_head.li(rvlib.A7, int(Sysno.WRITE))
    rvlib.ecall(p_head)
    p_head.li(20, 10)
    ...
    p_head.emit(rvasm.addi(rvlib.A0, rvlib.S0, 0))
    p_head.li(rvlib.A7, int(Sysno.CLOSE))
    rvlib.ecall(p_head)
    ...
    p_wc.li(rvlib.A1, 0)
    p_wc.li(rvlib.A7, int(Sysno.OPEN))
    rvlib.ecall(p_wc)
    p_wc.blt(rvlib.A0, 0, "wc_readfail")
    p_wc.emit(rvasm.addi(18, rvlib.A0, 0))
    p_wc.label("wc_loop")
    p_wc.emit(rvasm.addi(rvlib.A0, 18, 0))
    p_wc.li(rvlib.A1, wc_buf)
    p_wc.li(rvlib.A2, 256)
    p_wc.li(rvlib.A7, int(Sysno.READ))
    rvlib.ecall(p_wc)
    ...
    p_wc.emit(rvasm.add(rvlib.T2, rvlib.A1, rvlib.T1))
